...
J = sum((-1 * y .* log_hypo) - ((1 .- y) .* log_one_minus_hypo)) / m;
...
grad = (1 / m) * (X' * errors);
grad_regularization_terms = theta;
grad_regularization_terms(1) = 0; % because we don't add anything for j = 0
grad = grad + grad_regularization_terms * (lambda / m);
